...
public class NioPathDeserializer extends StdScalarDeserializer<Path> {
    ...
    public Path deserialize(JsonParser jsonParser, DeserializationContext deserializationContext) throws IOException {
        if (!jsonParser.hasToken(JsonToken.VALUE_STRING)) {
            return C14969a.m64318a(deserializationContext.handleUnexpectedToken((Class<?>) Path.class, jsonParser));
        }
        ...
        try {
            ...
            try {
                ...
                while (it.hasNext()) {
                    FileSystemProvider a = C14972d.m64321a(it.next());
                    if (a.getScheme().equalsIgnoreCase(scheme)) {
                        return a.getPath(uri);
                    }
                }
                return C14969a.m64318a(deserializationContext.handleInstantiationProblem(handledType(), text, e));
            } catch (Throwable th) {
                th.addSuppressed(e);
                return C14969a.m64318a(deserializationContext.handleInstantiationProblem(handledType(), text, th));
            }
        } catch (URISyntaxException e2) {
            return C14969a.m64318a(deserializationContext.handleInstantiationProblem(handledType(), text, e2));
        }
    }
}
