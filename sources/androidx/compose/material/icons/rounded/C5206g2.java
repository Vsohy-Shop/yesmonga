package androidx.compose.material.icons.rounded;

import androidx.compose.material.icons.C3053a;
import androidx.compose.p004ui.graphics.C15225h4;
import androidx.compose.p004ui.graphics.C15240j2;
import androidx.compose.p004ui.graphics.C15244j4;
import androidx.compose.p004ui.graphics.C15253k4;
import androidx.compose.p004ui.graphics.C15421z1;
import androidx.compose.p004ui.graphics.vector.C15340c;
import androidx.compose.p004ui.graphics.vector.C15348e;
import androidx.compose.p004ui.graphics.vector.C15380n;
import androidx.compose.p004ui.unit.C16483g;
import kotlin.jvm.internal.C11363r0;
import kotlin.jvm.internal.DefaultConstructorMarker;
import kotlin.jvm.internal.Intrinsics;
import org.jetbrains.annotations.C12579k;
import org.jetbrains.annotations.C12580l;

@C11363r0({"SMAP\nAnchor.kt\nKotlin\n*S Kotlin\n*F\n+ 1 Anchor.kt\nandroidx/compose/material/icons/rounded/AnchorKt\n+ 2 Icons.kt\nandroidx/compose/material/icons/IconsKt\n+ 3 Dp.kt\nandroidx/compose/ui/unit/DpKt\n+ 4 ImageVector.kt\nandroidx/compose/ui/graphics/vector/ImageVectorKt\n+ 5 Vector.kt\nandroidx/compose/ui/graphics/vector/VectorKt\n*L\n1#1,77:1\n122#2:78\n116#2,3:79\n119#2,3:83\n132#2,18:86\n152#2:123\n175#3:82\n694#4,2:104\n706#4,2:106\n708#4,11:112\n53#5,4:108\n*S KotlinDebug\n*F\n+ 1 Anchor.kt\nandroidx/compose/material/icons/rounded/AnchorKt\n*L\n29#1:78\n29#1:79,3\n29#1:83,3\n30#1:86,18\n30#1:123\n29#1:82\n30#1:104,2\n30#1:106,2\n30#1:112,11\n30#1:108,4\n*E\n"})
/* renamed from: androidx.compose.material.icons.rounded.g2 */
public final class C5206g2 {
    @C12580l

    /* renamed from: a */
    public static C15340c f12928a;

    @C12579k
    /* renamed from: a */
    public static final C15340c m19089a(@C12579k C3053a.C3056c cVar) {
        Intrinsics.checkNotNullParameter(cVar, "<this>");
        C15340c cVar2 = f12928a;
        if (cVar2 != null) {
            Intrinsics.checkNotNull(cVar2);
            return cVar2;
        }
        C15340c.C15341a aVar = r1;
        C15340c.C15341a aVar2 = new C15340c.C15341a("Rounded.Anchor", C16483g.m74435M(24.0f), C16483g.m74435M(24.0f), 24.0f, 24.0f, 0, 0, false, 224, (DefaultConstructorMarker) null);
        int c = C15380n.m67238c();
        C15225h4 h4Var = r0;
        C15225h4 h4Var2 = new C15225h4(C15240j2.f37547b.mo42841a(), (DefaultConstructorMarker) null);
        int a = C15244j4.f37564b.mo42862a();
        int a2 = C15253k4.f37575b.mo42879a();
        C15348e eVar = new C15348e();
        eVar.mo43276l(13.0f, 9.0f);
        eVar.mo43284t(7.82f);
        C15348e eVar2 = eVar;
        eVar2.mo43269e(14.16f, 7.4f, 15.0f, 6.3f, 15.0f, 5.0f);
        eVar2.mo43270f(0.0f, -1.65f, -1.35f, -3.0f, -3.0f, -3.0f);
        eVar.mo43280p(9.0f, 3.35f, 9.0f, 5.0f);
        C15348e eVar3 = eVar;
        eVar3.mo43270f(0.0f, 1.3f, 0.84f, 2.4f, 2.0f, 2.82f);
        eVar.mo43284t(9.0f);
        eVar.mo43272h(9.0f);
        eVar3.mo43270f(-0.55f, 0.0f, -1.0f, 0.45f, -1.0f, 1.0f);
        eVar.mo43285u(0.0f);
        eVar3.mo43270f(0.0f, 0.55f, 0.45f, 1.0f, 1.0f, 1.0f);
        eVar.mo43273i(2.0f);
        eVar.mo43285u(8.92f);
        eVar.mo43270f(-2.22f, -0.33f, -4.59f, -1.68f, -5.55f, -3.37f);
        eVar.mo43275k(1.14f, -1.14f);
        eVar.mo43270f(0.22f, -0.22f, 0.19f, -0.57f, -0.05f, -0.75f);
        eVar.mo43274j(3.8f, 12.6f);
        C15348e eVar4 = eVar;
        eVar4.mo43269e(3.47f, 12.35f, 3.0f, 12.59f, 3.0f, 13.0f);
        eVar.mo43285u(2.0f);
        eVar4.mo43270f(0.0f, 3.88f, 4.92f, 7.0f, 9.0f, 7.0f);
        eVar.mo43281q(9.0f, -3.12f, 9.0f, -7.0f);
        eVar.mo43285u(-2.0f);
        eVar.mo43270f(0.0f, -0.41f, -0.47f, -0.65f, -0.8f, -0.4f);
        eVar.mo43275k(-2.74f, 2.05f);
        C15348e eVar5 = eVar;
        eVar5.mo43270f(-0.24f, 0.18f, -0.27f, 0.54f, -0.05f, 0.75f);
        eVar.mo43275k(1.14f, 1.14f);
        eVar5.mo43270f(-0.96f, 1.69f, -3.33f, 3.04f, -5.55f, 3.37f);
        eVar.mo43284t(11.0f);
        eVar.mo43273i(2.0f);
        C15348e eVar6 = eVar;
        eVar6.mo43270f(0.55f, 0.0f, 1.0f, -0.45f, 1.0f, -1.0f);
        eVar.mo43285u(0.0f);
        eVar6.mo43270f(0.0f, -0.55f, -0.45f, -1.0f, -1.0f, -1.0f);
        eVar.mo43272h(13.0f);
        eVar.mo43268d();
        eVar.mo43276l(12.0f, 4.0f);
        eVar6.mo43270f(0.55f, 0.0f, 1.0f, 0.45f, 1.0f, 1.0f);
        eVar.mo43281q(-0.45f, 1.0f, -1.0f, 1.0f);
        eVar.mo43281q(-1.0f, -0.45f, -1.0f, -1.0f);
        eVar.mo43280p(11.45f, 4.0f, 12.0f, 4.0f);
        eVar.mo43268d();
        C15340c f = C15340c.C15341a.m66873d(aVar, eVar.mo43271g(), c, "", h4Var, 1.0f, (C15421z1) null, 1.0f, 1.0f, a, a2, 1.0f, 0.0f, 0.0f, 0.0f, 14336, (Object) null).mo43184f();
        f12928a = f;
        Intrinsics.checkNotNull(f);
        return f;
    }
}
